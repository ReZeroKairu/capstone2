...
const SubmissionHistory = ({
  ...
  const hasAcceptedLatest = manuscript.assignedReviewersMeta?.[currentUserId]?.acceptedAt;
  const isLatestVersion = (manuscript.submissionHistory?.length || 0) === manuscript.versionNumber;

  // Helper function to get reviewer display name based on role
  const getReviewerDisplayName = (reviewerId, reviewerNumber) => {
    if (role === 'Admin') {
      const reviewer = users.find(u => u.id === reviewerId);
      return reviewer ? `${reviewer.firstName} ${reviewer.lastName}` : `Reviewer ${reviewerId?.slice(-4) || ''}`;
    }
    return `Reviewer ${reviewerNumber || (reviewerId ? reviewerId.slice(-4) : '')}`;
  };

  // Gather all reviewer submissions (current and previous) per version
  const getReviewerSubmissionsForVersion = (versionNumber) => {
    const allSubmissions = [
      ...(manuscript.reviewerSubmissions || []).map(s => ({ ...s, isCurrent: true })),
      ...(manuscript.previousReviewSubmissions || []).map(s => ({ ...s, isCurrent: false }))
    ];
    ...
    return allSubmissions
      .filter((submission) => {
        ...
        const isUnderReview = ['Back to Admin', 'Under Review', 'Peer Reviewer Assigned'].includes(manuscript.status);
      ...
      manuscript.submissionHistory.length > 0 ? (
        manuscript.submissionHistory
          ...
          .filter((submission, idx) => {
            if (isReviewer && !hasAcceptedLatest && isLatestVersion && 
                              ...
                              >
                                <span className="font-medium text-xs">
                                  {users.find((u) => u.id === sub.reviewerId)
                                    ?.firstName || `Reviewer ${sub.reviewerId}`}
                                </span>
                          ...
                          return (
                            ...
                            >
                              <p className="text-xs font-medium text-gray-800">
                                {getReviewerDisplayName(sub.reviewerId, sub.reviewerNumber)} - 
                                Version {sub.manuscriptVersionNumber || versionNumber}
                                {sub.isLatest && " (Latest)"}
                              </p>
                              {sub.comment && (
