...
const Manuscripts = () => {
  ...
  const journalOptions = [
    ...
    {
      id: "ljher",
      value: "Liceo Journal of Higher Education Research (LJHER)",
    },
  ...
  const journalFilteredManuscripts = manuscripts.filter((m) => {
    ...
    return (
      journalQuestion?.answer ===
      journalOptions.find((j) => j.id === journalFilter)?.value
    );
  });
  ...
  const filteredManuscripts = journalFilteredManuscripts
    .filter((m) => {
      ...
      if (role === "reviewer") {
        ...
        const wasAssigned =
          (m.previousReviewers || []).includes(user.uid) ||
  ...
  // --- UI ---
  return (
    <div className="pt-24 px-4 sm:px-6 lg:px-24 mt-10 pb-36">
        ...
        {currentManuscripts.map((manuscript) => {
          ...
          return (
            <div
              ...
              className={
                isTargetManuscript ? "transition-all duration-500" : ""
              }
            >
              <ManuscriptItem
