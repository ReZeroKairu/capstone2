...
const ReviewerFeedback = ({
  ...
  return (
    <>
      {visibleReviewers.map((reviewer, reviewerIdx) => {
        ...
        const assignedByName = assignedByUser
          ? `${assignedByUser.firstName} ${assignedByUser.middleName ? assignedByUser.middleName + " " : ""}${assignedByUser.lastName}`
          : reviewer.assignedBy || "—";
    ...
    // In the ReviewerFeedback component, replace the submissionsToShow logic with:
// Determine which submissions to show
let submissionsToShow = allSubmissions;
let showNoFeedbackMessage = false;

if (role === "Researcher") {
  // For researchers, only show submissions that have been finalized
  submissionsToShow = allSubmissions.filter(submission => 
    submission.status === 'finalized' || 
    submission.isFinalized === true
  );
  
  // If no finalized submissions, set a flag to show a message
  showNoFeedbackMessage = submissionsToShow.length === 0;
  
  // If we want to show a message, we'll keep the reviewer card but show no submissions
  if (showNoFeedbackMessage) {
    submissionsToShow = []; // Empty array to prevent showing any submissions
  }
}
  ...
  >
    {role === "Admin" ? (
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <div className="text-xs text-gray-500 mt-1">
              Assigned: {formatDate(meta.assignedAt)} by {assignedByName}
              {meta.acceptedAt && (
                <div className="text-green-700 font-medium">
                  Accepted: {formatDate(normalizeTimestamp(meta.acceptedAt))}
                </div>
              )}
              {meta.declinedAt && (
                <div className="text-red-700 font-medium">
                  Declined: {formatDate(normalizeTimestamp(meta.declinedAt))}
                </div>
              )}
              {meta.deadline &&
                ![
                  ...
                  <div className="text-pink-700 font-medium">
                    Deadline: {formatDate(normalizeTimestamp(meta.deadline))}
                  </div>
                )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {decisionMeta?.decision ? (
              ...
              <span
                className={`text-xs px-2 py-1 rounded ${
                  !meta.invitationStatus || meta.invitationStatus === "pending"
            ...
            <button
              onClick={() => unassignReviewer(manuscript.id, reviewer.id)}
      ...
      // Researcher view - minimal information
      <div className="flex justify-between items-center">
        <div>
          <span className="text-blue-800 text-sm font-medium">
            {showNoFeedbackMessage ? "Review in progress" : "Review submitted"}
          </span>
          {!showNoFeedbackMessage && decisionMeta?.decision && (
            <div className="text-xs text-gray-500 mt-1">
              {decisionMeta.decision === "minor" && "Minor revisions requested"}
              {decisionMeta.decision === "major" && "Major revisions requested"}
              {decisionMeta.decision === "publication" && "Accepted for publication"}
              {decisionMeta.decision === "reject" && "Revisions required"}
            </div>
          )}
        </div>
                  ...
                  <div key={idx} className="border-t pt-2 first:border-0 first:pt-0">
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-semibold text-gray-800">
                        ...
                        {role !== "Researcher" && idx === allSubmissions.length - 1 ? " (Latest)" : ""}
                      </span>
