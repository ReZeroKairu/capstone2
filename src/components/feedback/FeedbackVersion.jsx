...
export const FeedbackVersion = ({
  ...
  return (
    <div className="feedback-version mb-4 transition-all duration-200 ease-in-out hover:shadow-md">
      <div 
        className={`feedback-version-header flex justify-between items-center p-4 rounded-lg ${isClickable ? 'cursor-pointer hover:bg-gray-50' : 'cursor-default opacity-80'}
          ${isExpanded ? 'bg-white border-b-0 rounded-b-none border border-gray-200' : 'bg-white border border-gray-200 shadow-sm'}`}
      ...
      >
        <div className="flex items-center space-x-3">
          <div className="flex items-center">
            <span className="text-lg font-semibold text-gray-800">Version {version}</span>
            {isLatestVersion && isAdmin && (
              <span className="ml-3 px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                Current
              </span>
            )}
            {isLatestVersion && !isAdmin && showAllFeedbackStatuses.includes(manuscriptStatus) && (
              <span className="ml-3 px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                Latest Feedback
              </span>
            )}
            {!isAdmin && isLatestVersion && !showAllFeedbackStatuses.includes(manuscriptStatus) && (
      ...
      <div 
        ...
        className={`feedback-version-content overflow-hidden transition-all duration-300 ease-in-out ${
          isExpanded ? 'max-h-[5000px] opacity-100 visible' : 'max-h-0 opacity-0 invisible'
        }`}
        style={{
