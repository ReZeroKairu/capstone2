    ...
    const unsubscribe = onSnapshot(
      ...
      (doc) => {
        if (doc.exists()) {
          ...
          setOriginalAnnouncements(defaultAnnouncements);
        }
        setLoading(false);
      },
      (error) => {
  ...
  useEffect(() => {
    if (isEditing) {
      announcements.forEach((_, idx) => {
        autoResize(messageRefs.current[idx]);
      });
    }
            ...
            {isEditing ? (
              <div className="flex flex-col items-center space-y-4">
                {announcements.map((a, index) => (
                  <div
                    key={index}
                    className="w-full mb-4 bg-red-900 p-4 rounded"
                  >
                    <input
                    ...
                    {announcements.map(
                      (a, index) =>
                        ...
                        a.message.trim() && (
                          <div
                            key={index}
                            className="bg-red-900 rounded p-4 mb-2 shadow"
                          >
