...
const SortableItem = ({ id, index, moveItem, children }) => {
  ...
  const handleScroll = useCallback((e) => {
    ...
    const scrollSpeed = Math.max(
      5,
      Math.floor(
        (scrollRegionHeight - Math.min(distanceFromTop, distanceFromBottom)) / 3
      )
    );
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      for (const a of announcements) {
        ...
        const base64Images =
          message.match(/data:image\/[^;]+;base64[^"]+/g) || [];

        if (base64Images.length > 0) {
          try {
            // Process and upload images
            const { content: processedContent } = await processHtmlImages(
              message
            );
        ...
        try {
          ...
          if (
            ...
            await notificationService.createBulkNotifications(
              ...
              "announcement",
              "New Announcement",
              "Check out the Announcements page to see what's new",
              {
      ...
      <div className="relative flex flex-col items-center mt-10 p-6 pt-28 pb-40 sm:p-16 sm:pt-28 sm:pb-16">
        <div className="relative w-full max-w-3xl bg-white border border-gray-300 rounded-lg shadow p-6 pt-11">
          ...
          <div
            ...
            className="announcements-container max-h-[60vh] overflow-y-auto"
          >
            {isEditing && (
              <div className="mb-4">
                <button
