          ...
          querySnapshot.docs.map(async (docRef) => {
            try {
              ...
              const abstractAnswer =
                data.answeredQuestions?.find(q => q.question === "Abstract")?.answer ||
                "No abstract available";

              // ✅ Define invitedAt first
            // Original invited time
const invitedAt = meta.invitedAt;

// Acceptance time (when reviewer accepted invitation)
...
if (respondedAt) {
  if (typeof respondedAt.toDate === "function") {
    displayAcceptedAt = new Date(respondedAt.toDate().getTime());
...
return {
  ...
  acceptedAt: displayAcceptedAt, // new field for sorting
  ...
  if (!a.acceptedAt) return 1;  // put pending at the bottom
  if (!b.acceptedAt) return -1;
  return b.acceptedAt - a.acceptedAt; // latest accepted first
});
  ...
  const handleDecision = async (manuscriptId, decision) => {
    ...
    try {
      ...
      const safeInvitedAt = invitation.originalInvitedAt || invitation.meta?.invitedAt || serverTimestamp();
...
if (adminIds.length > 0) {
  ...
  const AbstractModal = ({ isOpen, onClose, invitation }) => {
    ...
    const { title, abstract, keywords, invitedAt } = invitation;
              ...
              <div className="mt-3 text-center sm:mt-0 sm:text-left">
                ...
                <div className="mt-2">
                  <div className="mt-4 text-sm text-gray-500">
                    ...
                    {invitedAt.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true })}
                  </div>
                  <div className="mt-4">
                    <h4 className="text-sm font-medium text-gray-700">Abstract</h4>
                    <p className="mt-1 text-sm text-gray-600 whitespace-pre-line">{abstract}</p>
                  </div>
                  {keywords?.length > 0 && (
                    <div className="mt-4">
                      <h4 className="text-sm font-medium text-gray-700">Keywords</h4>
                      <div className="mt-2 flex flex-wrap gap-2">
                        {keywords.map((kw, idx) => <span key={idx} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{kw}</span>)}
                      </div>
                    </div>
                  )}
                </div>
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 pt-20 px-4">
        <div className="max-w-7xl mx-auto bg-red-50 border-l-4 border-red-400 p-4 text-red-700">{error}</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-20 px-4">
      <div className="max-w-7xl mx-auto pt-24">
        <h1 className="text-2xl font-bold text-gray-900 mb-4">Reviewer Invitations</h1>
        {invitations.length === 0 ? (
          <div className="bg-white rounded-lg shadow-sm p-12 text-center">
            <h3 className="text-sm font-medium text-gray-900">No pending invitations</h3>
            <p className="mt-1 text-sm text-gray-500">You'll see new review requests here when they're assigned to you.</p>
          </div>
        ) : (
          invitations.map(inv => {
            const { id, title, abstract, keywords, invitedAt } = inv;
            return (
              <div key={id} className="bg-white shadow overflow-hidden rounded-lg mb-4">
                <div className="px-6 py-5">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-medium text-gray-900">{title}</h3>
                    <span className="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">Pending Response</span>
                  </div>
                  <p className="mt-2 text-sm text-gray-600 line-clamp-2">{abstract}</p>
                  <div className="mt-4 text-sm text-gray-500">
                    <span className="font-medium">Invited: </span>
                    {invitedAt.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {keywords.slice(0, 3).map((kw, idx) => <span key={idx} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{kw}</span>)}
                    {keywords.length > 3 && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">+{keywords.length - 3} more</span>}
                  </div>
                </div>
                <div className="bg-gray-50 px-6 py-3 flex justify-between items-center">
                  <button type="button" onClick={() => handleViewDetails(inv)} className="text-sm font-medium text-blue-600 hover:text-blue-500">View Details</button>
                  <div className="space-x-3">
                    <button type="button" onClick={() => handleDecision(id, true)} disabled={processingId === id} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 disabled:opacity-50">
                      {processingId === id ? "Processing..." : "Accept"}
                    </button>
                    <button type="button" onClick={() => handleDecision(id, false)} disabled={processingId === id} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-red-600 hover:bg-red-700 disabled:opacity-50">
      ...
      <AbstractModal isOpen={!!viewingInvitation} onClose={handleCloseModal} invitation={viewingInvitation} />
    </div>
  );
};
